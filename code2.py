...
class GeneticAlgorithm:
    # set parameter
    def __init__(self, populNum, geneNum, selRatio, mutaNum, nowGeneration):
        ...
        self.chromoLists = [[0 for col in range(LENGTH)] for row in range(populNum)]
        self.fScoreLists = {}
        self.parent1 = 0
        self.parent2 = 0
        self.child1 = []
...
if __name__ == "__main__":
    ...
    userGenerate = int(input("Write a generation number: ")) # 세대 번호
    userPopulation = int(input("Write a population number: ")) # 모집단 번호
    userTournaRatio = float(input("Write a tournament selection ratio number: ")) # 토너먼트 선택 비율
    userMutaRatio = float(input("Write a mutation ratio number: ")) # 돌연변이 비율
